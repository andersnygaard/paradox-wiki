---
type: Concept
title: Stem
description: Describes the Paradox's bow stem — its timber, thickness, notching, and assembly into the hull — including why its 100 mm thickness protects the boat during sculling-oar collisions.
tags: [paradox, components, stem, bow, structure]
timestamp: 2026-07-20T00:00:00Z
---

# Stem

The Paradox's bow stem: the timber it is made from, its notching, and how it is assembled into the hull.

The stem post is 100 mm (4") thick. A sculling oar has no reverse and no brake, so contact with an obstruction while sculling is unavoidable — the stem's thickness is what leaves the boat undamaged when it happens.[1]

## Where this is covered

- [Notching the stem for chine logs and sheer clamps](/construction/stem-notching.md) — how the chine logs and sheer clamps are inlet into the stem.

# Citations

[1] ["Sailing The Paradox"](/references/dca-sailing-the-paradox.md), *DCA Bulletin* 193: 48-49 (Dinghy Cruising Association), Alastair Law — supports the stem post's 100 mm (4") thickness and its role in absorbing sculling-oar collisions without damage to the boat.
